...
public class Bib {
    ...
    public static void botonFecha(ArrayList<Duelo> fechaListDuelos, JLabel[][] tablaLabels, JPanel tabla, int fecha, JButton[] botonesFechas, Grupo group){ //contenido al presionar el boton de la fecha
        ...
        generarPartidos(pPartidos, calcular, azarBtn, fechaListDuelos, tablaLabels, tabla, nuevaFechaFrame, fecha, botonesFechas, rules, group);

        nuevaFechaFrame.add(pPartidos, BorderLayout.NORTH);
        nuevaFechaFrame.setVisible(true);
    }

    public static void generarPartidos(JPanel pPartidos, JButton calcular, JButton azrBtn, ArrayList<Duelo> fechaNNpartidos, JLabel[][] tablaLabels, JPanel tabla, JFrame nuevaFechaFrame, int fecha, JButton[] botonesFechas, GridBagConstraints rules, Grupo group) {
        ...
        azrBtn.addActionListener(e->{
            if (group.getEquipoList().get(0).getPartidosJugados() == 18) {
                azrBtn.setEnabled(false);
                JOptionPane.showMessageDialog(null, "No se pueden Re-Jugar los partidos", "Excepcion de entrada", JOptionPane.ERROR_MESSAGE);
            } else {
                System.out.println("---------------");
                    for (Duelo dl : fechaNNpartidos) {
                        if (dl.getgLoc() == -1) {
                            int[] goles = Equipo.simularPartido(dl.getlocal(), dl.getvisitante());
                            Bib.asignarGolesADuelo(dl.getlocal(), dl.getvisitante(), group.getCalendarioList().indexOf(fechaNNpartidos), goles[0], goles[1], group.getCalendarioList());
